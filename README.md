...
## LARMORD-Q: 
Using a database of DFT-derived shielding values, parameters were trained that now enables users to compute DFT-like shielding values directly from structure. To compute DFT-like shielding values from structure, users just need to specify the appropriate parameter and reference files. The ```-residueBased``` is needed as well (see example below).
